...
const events = [
  ...
  { text: "นายจ้างโกงไม่จ่ายเงินเดือน!", noSalary: true, cost: 0 },
...
function nextMonth() {
  ...
  players.forEach(player => {
    ...
    if (randomEvent.noSalary) {
      ...
      player.paymentEl.disabled = false;
    }
    ...
    if (pay > player.money) {
      eventText += ` | จ่ายหนี้เกินตัว! ยอดหนี้เพิ่มขึ้น ${deficit}`;
      monthSummaries.push({ player, summary: `<div>เดือน ${month}: ${eventText} | จ่ายหนี้ ${pay} | เหลือเงินเก็บ: ${player.money} | ยังมีหนี้อีก: ${player.debt}</div>` });
      players.forEach(p => updateStats(p));
      let winner = players.find(p => p !== player);
      endGame(winner, "ชนะ");
      gameEnded = true;
      return;
    }
  ...
  monthSummaries.forEach(s => {
    ...
    s.player.historyEl.scrollTop = s.player.historyEl.scrollHeight; // ✅ เลื่อนอัตโนมัติ
  });
  players.forEach(p => updateStats(p));
...
function endGame(winner, result) {
  ...
  let loseReason;
  if (loser.historyEl.lastChild.textContent.includes("จ่ายหนี้เกินตัว")) {
    loseReason = "แพ้เพราะจ่ายหนี้เกินตัว!";
  } else if (loser.money < 0) {
  ...
  let winMsg = winner.debt <= 0 ?
    ...
    `${winner.name} ชนะเพราะอีกฝ่ายแพ้! แต่คุณยังมีหนี้เหลืออยู่นะ รีบเคลียร์ให้หมด!`;
  let loseMsg = `${loser.name} ${loseReason} แต่ชีวิตจริงขอให้คุณไม่เป็นหนี้ และบริหารจัดการเงินได้อย่างมีความสุขนะ.`;
